...
import { Search, Filter, Download, TrendingUp, Calendar, Database, Eye } from "lucide-react";
...
export default function ExploreClient({ symbol }: ExploreClientProps) {
  ...
  // If a specific symbol is requested, show symbol page
  if (symbol) {
    const tickerData = data?.tickers.find(t => t.ticker.toUpperCase() === symbol.toUpperCase());
    ...
    return (
      <div className="container mx-auto px-6 py-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-white mb-2">
                {symbol.toUpperCase()} {tickerData?.name && `- ${tickerData.name}`}
              </h1>
              <p className="text-gray-400">
                Symbol details and price chart
              </p>
            </div>
            <Link
        ...
        {tickerData && (
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
            <div className="bg-gray-800 rounded-lg p-4">
              <div className="text-sm text-gray-400 mb-1">Records</div>
              <div className="text-2xl font-bold text-white">
                {tickerData.records?.toLocaleString() ?? "N/A"}
              </div>
            </div>
            <div className="bg-gray-800 rounded-lg p-4">
              <div className="text-sm text-gray-400 mb-1">Sector</div>
              <div className="text-2xl font-bold text-white">
                {tickerData.sector ?? "N/A"}
              </div>
            </div>
            <div className="bg-gray-800 rounded-lg p-4">
              <div className="text-sm text-gray-400 mb-1">First Date</div>
              <div className="text-2xl font-bold text-white">
                {tickerData.firstDate ?? "N/A"}
              </div>
            </div>
            <div className="bg-gray-800 rounded-lg p-4">
              <div className="text-sm text-gray-400 mb-1">Last Date</div>
              <div className="text-2xl font-bold text-white">
                {tickerData.lastDate ?? "N/A"}
              </div>
            </div>
          </div>
        )}
        ...
        <div className="bg-gray-800 rounded-lg p-6">
          ...
          <PriceChart ticker={symbol.toUpperCase()} />
        </div>
      </div>
    );
  }
                ...
                <tr key={ticker.ticker} className={index % 2 === 0 ? "bg-gray-750" : "bg-gray-800"}>
                  ...
                  <td className="p-4">
                    <div className="flex space-x-2 justify-center">
                      <Link
                        href={`/explore?symbol=${ticker.ticker}`}
