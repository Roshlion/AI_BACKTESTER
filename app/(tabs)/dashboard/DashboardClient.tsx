...
import { buildPromptFromIndicators } from "@/lib/strategy-selection";
import { commitStrategyNavigation } from "./strategy-hand-off";
...
export default function DashboardClient() {
  ...
  const { setSelection } = useStrategyState();
  ...
  const indicatorParams = useMemo(() => {
    const params: string[] = [];
    if (showSMA) params.push(`SMA${smaPeriod}`);
    if (showEMA) params.push(`EMA${emaPeriod}`);
    if (showRSI) params.push("RSI");
    if (showMACD) params.push("MACD");
    return params;
  }, [showSMA, showEMA, showRSI, showMACD, smaPeriod, emaPeriod]);

  const promptSuggestion = useMemo(
    () => buildPromptFromIndicators(indicatorParams),
    [indicatorParams],
  );
  ...
  return (
    <div className="flex w-full flex-col gap-8">
      <div className="flex flex-col gap-8 rounded-xl border border-white/10 bg-slate-900/60 px-4 py-6 shadow-lg sm:px-6">
